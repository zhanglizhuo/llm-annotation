...
resolve_python_bin() {
  local preferred="${QWEN35_PYTHON_BIN:-/root/anaconda3/envs/higentec_llava/bin/python}"
  if [[ -x "$preferred" ]]; then
    echo "$preferred"
    return
  fi
...
LOG=${LOG:-logs/cross_model_validation/qwen35_dual_${STAMP}.log}
PIDFILE=${PIDFILE:-logs/cross_model_validation/qwen35_dual_${STAMP}.pid}

MODEL_SEQUENCE=${MODEL_SEQUENCE:-"qwen35_27b qwen35_35b_a3b"}
...
run_worker() {
  echo "[$(date '+%F %T')] Qwen3.5 dual run start"
  ...
  echo "[$(date '+%F %T')] Qwen3.5 dual run finished"
}
...
echo "started Qwen3.5 dual-model HF background run"
echo "pid: $PID"
