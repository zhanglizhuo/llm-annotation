...
\author{\uppercase{Yan Ma}\authorrefmark{1}\authorrefmark{2} and \uppercase{Lizhuo Zhang}\authorrefmark{3}}

\address[1]{School of Foreign Studies, Changsha University of Science and Technology, Changsha, China}
...
analysis, and a ten-model cross-model robustness check spanning
...
respectively) while \textit{answer} remains below 19\% universally,
...
A ten-model robustness analysis (expanded from five models in the original submission) is treated as a boundary
...
\begin{enumerate}
  ...
  ten-model robustness check bound the claim and point to label quality
...
\noindent\textbf{Annotation stage:} YOLO-detected bounding-box crops are
...
\noindent\textbf{Filtering stage:} Four filtering strategies are compared:
...
\noindent\textbf{Fine-tuning stage:} CLIP ViT-L/14 is fine-tuned under
...
cross-model robustness check---read frozen artifacts from
...
the index only. The complete label-index mapping for each sub-dataset
...
\noindent\textbf{None (unfiltered):} All crops for which Qwen
produces a valid index are retained.
The pipeline uses the Qwen prediction as the pseudo-label.

\noindent\textbf{Valid-Output-Qwen:} Only crops where Qwen outputs
a valid in-range index are retained (identical to None in
practice, as Qwen has a near-100\% valid-output rate).

\noindent\textbf{Valid-Output-LLaVA:} Only crops where LLaVA outputs
a valid index are retained.

\noindent\textbf{Agreement:} Only crops where Qwen and LLaVA assign
...
\noindent\textbf{None:} Unfiltered MLLM pseudo-labels (Qwen predictions).

\noindent\textbf{Agreement:} Agreement-filtered pseudo-labels.

\noindent\textbf{GT:} Ground-truth human annotations (supervised reference).
...
revised study includes an expanded cross-model robustness check with ten
open-weight MLLMs spanning three model families (Qwen2, Qwen2.5, Qwen3.5,
...
cross-model consistency (CMC) score computed from the ten MLLM
...
for the ten tested MLLMs under the common bbox-crop and index-only prompting
protocol.
...
Third, the \textit{answer} category remains below 19\% across all ten models
...
\textit{Answer} remains a near-universal failure: all ten models score
...
\begin{table}[!ht]
\caption{Ten-model cross-model validation on four key TeacherBehavior
categories illustrating the capability boundary (\textit{guide}) and the
...
\begin{tabular}{lccc}
\toprule
\textbf{Training condition} & \textbf{Qwen2 (41.2\%)} & \textbf{Qwen3.5-27B (50.3\%)} & $\boldsymbol{\Delta}$ \\
\midrule
...
\midrule
\emph{Prompt-ensemble ZS} & \emph{53.43} & \emph{53.43} & --- \\
\emph{GT + LP} & \emph{69.50} & \emph{---} & --- \\
\bottomrule
\end{tabular}
}
\end{table}
...
The ten-model boundary check strengthens this interpretation.
...
ten models across three families (Qwen, LLaVA, Gemma): Qwen2-VL-7B, LLaVA-1.5-7B,
...
The ten-model robustness analysis and the Qwen3.5-27B quality-threshold
...
The analysis modules described in this paper are implemented as the
following scripts at the repository root:
\path{step0b_cape_zeroshot.py} (zero-shot evaluation),
\path{step1_llm_annotate.py} and \path{cross_model_annotate.py}
(MLLM annotation),
\path{step2_filter_analysis.py} (filtering analysis),
\path{step3_clip_finetune.py} (CLIP fine-tuning),
\path{step4_selective_annotation.py} (selective annotation),
\path{step5_retention_curve.py} (retention-ratio curve),
and \path{summarize_repeated_seeds.py} (repeated-seed aggregation).
Auxiliary analyses are implemented in
\path{confidence_filtering.py},
\path{teacher_student_self_training.py},
and \path{cross_model_consistency.py}.
The runtime environment is specified in \path{environment.yml}
and \path{requirements.txt}.
Table-level provenance: annotation-quality and confusion tables are
produced by the filtering-analysis module; the main fine-tuning matrix
by the fine-tuning and repeated-seed summary modules; the
selective-routing results by the selective-annotation module; the
auxiliary-baseline results by the confidence-filtering and
...
The complete codebase, run configurations, and generated result files
are archived at
\url{https://github.com/zhanglizhuo/llm-annotation}
(accessed on 25 June 2026), pinned at commit \texttt{e21ae3d}.
...
are single-seed or fixed-seed evaluations (seed 42).
...
\textit{guide} while confirming that \textit{answer} remains below 19\%
...
ten-model robustness check then bound the claim: pseudo-label
...
25 June 2026). These datasets are externally released resources and are
not owned by this study. The experiment code and archived run
configurations are available at
\url{https://github.com/zhanglizhuo/llm-annotation} (accessed on 25 June 2026).
...
is an Associate Professor at the School of Foreign Studies, Changsha
...
received the B.S. degree in Horticulture from Hunan Agricultural
