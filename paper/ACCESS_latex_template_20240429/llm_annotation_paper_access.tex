...
\begin{abstract}
Agreement-based pseudo-label filtering is often treated as a reliability
heuristic for multimodal large language models~(MLLMs), but its
downstream value in fine-grained classroom behavior recognition is
uncertain. This study presents a reproducible reliability-audit protocol
for testing that assumption on three SCB sub-datasets. The protocol links
bounding-box-level annotation-quality measurement, repeated-seed CLIP
fine-tuning, retention-ratio controls, visual-anchoring diagnostics,
auxiliary filtering and self-training baselines, and a five-model
robustness check.

Under this single-frame SCB protocol, the audit shows that agreement
filtering is not a portable denoising rule. On BowTurnHead,
agreement-retained linear probing falls to
15.53\% $\pm$ 1.00\%, whereas unfiltered pseudo-label supervision reaches
88.02\% $\pm$ 0.12\%; diagnostic retention controls indicate that this
collapse cannot be explained by sample count alone. Across datasets, the
observed failures are consistent with three interacting risks:
data-volume loss, class-composition shift, and a label-quality ceiling.

Visual anchoring provides a task-internal screening signal for
interpreting these differences. Protocol-matched per-class zero-shot
CLIP accuracy is used as an operational anchor proxy, not as an
...
supervision could be useful if a deployable router were available.
Auxiliary strategy baselines reinforce the same conditional pattern:
...
Across five tested MLLMs, no annotator is uniformly best across datasets.
Targeted five-seed LoRA replications show lower cross-seed variability
under ground-truth than pseudo-label supervision (mean SD = 0.107\% vs.
0.495\%), supporting label quality as a major constraint. The result is
an auditable protocol and evidence base for deciding when MLLM
pseudo-labels are likely to provide useful supervision.
\end{abstract}
...
A five-model robustness analysis is treated as a boundary
...
\begin{enumerate}
  ...
  five-model robustness check bound the claim and point to label quality
...
Class balance is uneven in all three sub-datasets and is reported
explicitly in Table~\ref{tab:class_balance}.
...
cross-model consistency (CMC) score computed from the five MLLM
...
not a model-independent denoising effect.
...
difficult to resolve from single-frame crops under this task
setup.
...
be inadequate.
Under oracle routing, assigning high-anchor categories to pseudo-label
supervision while leaving low-anchor categories to the zero-shot branch
scores above both full pseudo-label fine-tuning and the zero-shot
baseline.
The result should still be read as a boundary diagnostic, not a
deployable method.
...
five-model robustness check then bound the claim: pseudo-label
...
Deitke M, Clark C, Lee S, Tripathi S, Yang Y, Park J, et al.
Molmo and PixMo: Open weights and open data for state-of-the-art
multimodal models. arXiv [Preprint]. 2024 [cited 2026 May 7]. Available
