...
\begin{abstract}
\textbf{Problem:} Dual-model agreement is widely used as a pseudo-label
reliability heuristic for multimodal large language models (MLLMs), but
whether agreement-filtered labels improve downstream fine-tuning in
fine-grained classroom behavior recognition remains unknown.

\textbf{Study design:} This work evaluates the downstream value of
agreement filtering under a reproducible single-frame protocol on three
SCB classroom behavior sub-datasets (BowTurnHead, HandriseReadWrite,
TeacherBehavior). The protocol chains bounding-box-level MLLM annotation
quality measurement, four filtering strategies, repeated-seed CLIP
fine-tuning (linear probe and LoRA), retention-ratio diagnostics,
selective-routing analysis, and a ten-model cross-model robustness check
spanning Qwen2, Qwen2.5, Qwen3.5, Qwen3.6, LLaVA, Gemma3, and Gemma4
families across 7B--35B parameter scales.

\textbf{Main findings:} (1) Agreement filtering is not a portable
...
$88.02\% \pm 0.12\%$; retention-ratio controls confirm that this
collapse is not explained by sample count alone but by class-composition
shift and a label-quality ceiling. (2) The operative constraint is visual
anchoring---the degree to which a category can be recovered from a single
frame without discourse context. Categories with low zero-shot CLIP
accuracy (e.g., \textit{guide}, \textit{answer}) systematically fail
across all tested MLLMs, while visually anchored categories remain
reliably annotated. A ten-model cross-model validation confirms that
newer-generation models (Qwen3.5-27B, Gemma4-31B) substantially improve
on \textit{guide} (39--83\%) while \textit{answer} remains below 23\%
universally, identifying a task-level single-frame observability
boundary. (3) A quality-threshold experiment using Qwen3.5-27B
pseudo-labels (annotation accuracy 50.3\% vs.\ Qwen2-VL-7B's 41.2\%)
crosses the critical threshold at which pseudo-label fine-tuning begins
to exceed the zero-shot baseline ($55.29\%$ vs.\ $53.43\%$), pinpointing
the required annotation quality for practical deployment.

\textbf{Implication:} Agreement filtering alone is insufficient as a
universal pseudo-label quality guarantee for MLLM-assisted annotation in
classroom behavior recognition. Downstream utility is gated by per-class
visual anchoring and annotation quality thresholds, which together
provide a task-internal screening signal for deciding when MLLM
...
open-weight MLLMs spanning four model families (Qwen2, Qwen2.5, Qwen3.5,
...
Second, a clear generational boundary exists on the \textit{guide} category:
models released before 2024 (Qwen2, Qwen2.5, LLaVA, Gemma3) all score below
25\%, while newer models (Qwen3.5-27B, Gemma4-31B) achieve 39--83\%,
showing that \textit{guide} is a model-capacity bottleneck rather than a
task-level impossibility.
Third, the \textit{answer} category remains below 23\% across all ten models
...
Abbreviations: Q3.5-27B = Qwen3.5-27B; Q3.5-35B = Qwen3.5-35B-A3B (MoE);
...
The \textit{guide} category shows a clear generational divide: pre-2024
models (Qwen2, Qwen2.5, LLaVA, Gemma3) all score below 1\% except LLaVA
...
annotator on TeacherBehavior (overall accuracy 52.19\%), substantially
outperforming the original Qwen2-VL-7B annotator (41.21\%).
