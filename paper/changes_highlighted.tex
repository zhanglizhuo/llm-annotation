...
%DIF DEL /home/lizhuo/Work/Mayan/llm-annotation/paper/llm_annotation_paper_access.tex           Thu Jun 25 06:25:53 2026
%DIF ADD /home/lizhuo/Work/Mayan/llm-annotation/paper/llm_annotation_paper_access_revised.tex   Thu Jun 25 07:31:07 2026
\usepackage{cite}
...
\author{\uppercase{Yan Ma}\authorrefmark{1}\DIFdelbegin %DIFDELCMD < \authorrefmark{3} %%%
\DIFdelend \DIFaddbegin \authorrefmark{2} \DIFaddend and \uppercase{Lizhuo Zhang}\DIFdelbegin %DIFDELCMD < \authorrefmark{2}%%%
\DIFdelend \DIFaddbegin \authorrefmark{3}\DIFaddend }

\DIFdelbegin %DIFDELCMD < \address[1]{School of Education, Hunan Agricultural University, Changsha, China}
%DIFDELCMD < \address[2]{School of Information and Intelligent Science and Technology, Hunan Agricultural University, Changsha, China}
%DIFDELCMD < \address[3]{School of Foreign Studies, Changsha University of Science and Technology, Changsha, China}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \address[1]{School of Foreign Studies, Changsha University of Science and Technology, Changsha, China}
\address[2]{School of Education, Hunan Agricultural University, Changsha, China}
\address[3]{School of Information and Intelligent Science and Technology, Hunan Agricultural University, Changsha, China}
\DIFaddend \tfootnote{This work was supported by the Education Department of Hunan Province under Grant HNJG-2022-0608.}
...
\begin{abstract}
\DIFdelbegin \DIFdel{Agreement-based }\DIFdelend \DIFaddbegin \DIFadd{Dual-model agreement is widely used as a }\DIFaddend pseudo-label \DIFdelbegin \DIFdel{filtering is often treated as a }\DIFdelend reliability
heuristic for multimodal large language models \DIFdelbegin \DIFdel{~}\DIFdelend (MLLMs), but \DIFdelbegin \DIFdel{its
downstream value }\DIFdelend \DIFaddbegin \DIFadd{whether
agreement-filtered labels improve downstream fine-tuning }\DIFaddend in fine-grained
classroom behavior recognition \DIFdelbegin \DIFdel{is
}\DIFdelend \DIFaddbegin \DIFadd{remains }\DIFaddend uncertain.
This study presents a reproducible reliability-audit protocol \DIFdelbegin \DIFdel{for testing that
assumption }\DIFdelend \DIFaddbegin \DIFadd{that
evaluates agreement filtering under a single-frame protocol }\DIFaddend on three SCB
\DIFaddbegin \DIFadd{classroom behavior }\DIFaddend sub-datasets \DIFdelbegin \DIFdel{. The protocol links
}\DIFdelend \DIFaddbegin \DIFadd{(BowTurnHead, HandriseReadWrite,
TeacherBehavior), linking }\DIFaddend bounding-box-level \DIFdelbegin \DIFdel{annotation-quality measurement, }\DIFdelend \DIFaddbegin \DIFadd{MLLM annotation quality
measurement, four filtering strategies, }\DIFaddend repeated-seed CLIP fine-tuning
\DIFaddbegin \DIFadd{(linear probe and LoRA)}\DIFaddend , retention-ratio \DIFdelbegin \DIFdel{controls, visual-anchoring diagnostics, auxiliary filtering and self-training baselines}\DIFdelend \DIFaddbegin \DIFadd{diagnostics, selective-routing
analysis}\DIFaddend , and a \DIFdelbegin \DIFdel{five-model
robustness check }\DIFdelend \DIFaddbegin \DIFadd{ten-model cross-model robustness check spanning
three model families (Qwen2, Qwen2.5, Qwen3.5, Qwen3.6, LLaVA, Gemma3,
and Gemma4) across 7B--35B parameter scales}\DIFaddend .

\DIFdelbegin \DIFdel{Under this single-frame SCB protocol, the audit shows that }\DIFdelend \DIFaddbegin \DIFadd{Three main findings emerge. First, }\DIFaddend agreement filtering is not a portable
denoising rule\DIFdelbegin \DIFdel{. On }\DIFdelend \DIFaddbegin \DIFadd{: on }\DIFaddend BowTurnHead, agreement-retained linear probing falls
to \DIFdelbegin \DIFdel{15.53\% $\pm$ 1.00\%, whereas unfiltered pseudo-label supervision reaches
88.02\% $\pm$ 0.12\%; diagnostic retention controls indicate }\DIFdelend \DIFaddbegin \DIFadd{$15.53\% \pm 1.00\%$ while unfiltered pseudo-labels reach
$88.02\% \pm 0.12\%$, and retention-ratio controls confirm }\DIFaddend that this
collapse \DIFdelbegin \DIFdel{cannot be explained by sample count alone. Across datasets, the
observed failures are consistent with three interacting risks:
data-volume loss, }\DIFdelend \DIFaddbegin \DIFadd{reflects }\DIFaddend class-composition shift \DIFdelbegin \DIFdel{, }\DIFdelend and a label-quality ceiling
\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Visual anchoring provides a task-internal screening signal for
interpreting these differences. Protocol-matched per-class }\DIFdelend \DIFaddbegin \DIFadd{rather than sample count alone. Second, the operative constraint is
visual anchoring---the degree to which a category can be recovered from a
single frame without discourse context. Categories with low }\DIFaddend zero-shot
CLIP accuracy \DIFdelbegin \DIFdel{is used as an operational anchor proxy, not as an
independent causal measure.An oracle selective-routing experiment is
reported only as an upper-bound diagnostic, showing where anchor-aware
supervision could be useful if a deployable router were available.Auxiliary strategy baselines reinforce the same conditional pattern:
confidence filtering matches unfiltered pseudo-label supervision on the
two visually grounded student-behavior datasets but yields no usable
TeacherBehavior subset under the evaluated confidence grid, whereas a
train-split teacher--student baseline substantially improves
TeacherBehavior while underperforming on HandriseReadWrite.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Across five tested MLLMs}\DIFdelend \DIFaddbegin \DIFadd{(e.g.}\DIFaddend , \DIFdelbegin \DIFdel{no annotator is uniformly best across datasets. Targeted five-seed LoRA replications show lower cross-seed variability
under ground-truth than pseudo-label supervision (mean SD = 0.107}\DIFdelend \DIFaddbegin \textit{\DIFadd{guide}}\DIFadd{, }\textit{\DIFadd{answer}}\DIFadd{) systematically fail
across all tested MLLMs; newer-generation models (Qwen3.5-27B,
Gemma4-31B) substantially improve on }\textit{\DIFadd{guide}} \DIFadd{(53\% and 83\%
respectively) while }\textit{\DIFadd{answer}} \DIFadd{remains below 19\% universally,
identifying a task-level single-frame observability boundary. Third, a quality-threshold
experiment with Qwen3.5-27B pseudo-labels (annotation accuracy 50.3}\DIFaddend \%
vs.\DIFdelbegin \DIFdel{0.495\%) , supporting label quality as a major constraint.The result is an auditable protocol and
evidence base }\DIFdelend \DIFaddbegin \DIFadd{\ Qwen2-VL-7B's 41.2\%) locates the critical annotation accuracy range
...
signal }\DIFaddend for deciding when MLLM pseudo-labels are likely to provide useful
supervision \DIFaddbegin \DIFadd{in classroom behavior recognition}\DIFaddend .
\end{abstract}
...
\DIFaddbegin \DIFadd{Formally, visual anchoring is defined as the degree to which a
...
\DIFadd{The practical motivation for studying agreement filtering deserves
explicit statement.
...
crowdsourcing~\mbox{%DIFAUXCMD
\cite{snow2008cheap} }\hskip0pt%DIFAUXCMD
and consistency-based semi-supervised
learning~\mbox{%DIFAUXCMD
\cite{laine2016temporal}}\hskip0pt%DIFAUXCMD
---has made dual-model agreement a widely
...
A \DIFdelbegin \DIFdel{five-model robustness analysis }\DIFdelend \DIFaddbegin \DIFadd{ten-model robustness analysis (expanded from five models in the original submission) }\DIFaddend is treated as a boundary
...
\begin{enumerate}
  ...
  \DIFdelbegin \DIFdel{five-model }\DIFdelend \DIFaddbegin \DIFadd{ten-model }\DIFaddend robustness check bound the claim and point to label quality
...
\DIFdelbegin \DIFdel{The study is organised as a top-down evaluation pipeline aligned with
the three research questions stated in the Introduction.
It first measures bbox-level }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Pipeline Overview}}

\DIFadd{The experimental pipeline consists of three stages, each producing
fixed artifacts consumed by downstream analyses:
}

\noindent\DIFadd{\textbf{Annotation stage:}} \DIFadd{YOLO-detected bounding-box crops are
...
\noindent\DIFadd{\textbf{Filtering stage:}} \DIFadd{Four filtering strategies are compared:
...
strategies produce training }\DIFaddend pseudo-label \DIFdelbegin \DIFdel{reliability under a common
MLLM annotation protocol, then tests whether filtered or unfiltered
pseudo-labels improve downstream }\DIFdelend \DIFaddbegin \DIFadd{JSONL files for the downstream
stage.
}

\noindent\DIFadd{\textbf{Fine-tuning stage:}} \DIFaddend CLIP \DIFdelbegin \DIFdel{fine-tuning relative to a training-free }\DIFdelend \DIFaddbegin \DIFadd{ViT-L/14 is fine-tuned under
...
}\DIFaddend zero-shot \DIFdelbegin \DIFdel{reference, and finally uses targeted diagnostic
analyses to explain the observed gains and failure modes .
These linked analyses include category-level anchoring diagnostics}\DIFdelend \DIFaddbegin \DIFadd{baselines. Four diagnostic modules---selective routing}\DIFaddend ,
retention-ratio \DIFdelbegin \DIFdel{tests for data-volume effects, a category-aware routing
diagnostic on TeacherBehavior}\DIFdelend \DIFaddbegin \DIFadd{curves, strategy audits (confidence filtering,
teacher--student self-training, cross-model consistency)}\DIFaddend , and a
cross-model robustness \DIFdelbegin \DIFdel{check to determine whether the main annotation patterns persist under model
substitution}\DIFdelend \DIFaddbegin \DIFadd{check---read frozen artifacts from
earlier stages to isolate specific mechanisms}\DIFaddend .
...
\DIFaddendFL \caption{Overview of the three SCB sub-datasets used in this study\DIFaddbeginFL \DIFaddFL{,
including per-class bounding-box counts}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{``BB'' = bounding boxes}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Class imbalance interacts with agreement filtering and anchor-aware routing
(Sec.~\ref{sec:discussion})}\DIFaddendFL .}
\label{tab:datasets}
\centering
\DIFdelbeginFL %DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD < \begin{tabular}{lcccc}
%DIFDELCMD < \toprule
%DIFDELCMD < \textbf{Sub-dataset} & \textbf{Classes} & \textbf{Train images} &
%DIFDELCMD < \textbf{Val images} & \textbf{Total BB (train/val)} \\
%DIFDELCMD < \midrule
%DIFDELCMD < BowTurnHead       & 2 & 1{,}905 & 505  & 12{,}365 / 3{,}753  \\
%DIFDELCMD < HandriseReadWrite & 3 & 5{,}193 & 1{,}671 & 34{,}524 / 12{,}848 \\
%DIFDELCMD < TeacherBehavior   & 8 & 8{,}984 & 3{,}240 & 40{,}372 / 15{,}327 \\
...
Class balance is uneven in all three sub-datasets\DIFdelbegin \DIFdel{and is reported
explicitly in Table~\ref{tab:class_balance}}\DIFdelend .
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Per-class bounding-box counts for the SCB sub-datasets.
These counts expose the class imbalance that interacts with agreement
filtering and anchor-aware routing.}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:class_balance}
%DIFDELCMD < \centering
%DIFDELCMD < \begin{tabular}{llrr}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdelFL{\textbf{Sub-dataset}} %DIFAUXCMD
%DIFDELCMD < & %%%
\DIFdelFL{\textbf{Class}} %DIFAUXCMD
%DIFDELCMD < & %%%
\DIFdelFL{\textbf{Train BB}} %DIFAUXCMD
%DIFDELCMD < & %%%
\DIFdelFL{\textbf{Val BB}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
\DIFdelFL{BowTurnHead }%DIFDELCMD < & %%%
\DIFdelFL{bow-head }%DIFDELCMD < & %%%
\DIFdelFL{4}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{422 }%DIFDELCMD < & %%%
\DIFdelFL{540 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{BowTurnHead }%DIFDELCMD < & %%%
\DIFdelFL{turn-head }%DIFDELCMD < & %%%
\DIFdelFL{7}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{943 }%DIFDELCMD < & %%%
\DIFdelFL{3}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{213 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{HandriseReadWrite }%DIFDELCMD < & %%%
\DIFdelFL{hand-raise }%DIFDELCMD < & %%%
\DIFdelFL{10}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{538 }%DIFDELCMD < & %%%
\DIFdelFL{2}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{915 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{HandriseReadWrite }%DIFDELCMD < & %%%
\DIFdelFL{read }%DIFDELCMD < & %%%
\DIFdelFL{17}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{539 }%DIFDELCMD < & %%%
\DIFdelFL{6}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{539 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{HandriseReadWrite }%DIFDELCMD < & %%%
\DIFdelFL{write }%DIFDELCMD < & %%%
\DIFdelFL{6}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{447 }%DIFDELCMD < & %%%
\DIFdelFL{3}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{394 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{guide }%DIFDELCMD < & %%%
...
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{answer }%DIFDELCMD < & %%%
\DIFdelFL{2}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{574 }%DIFDELCMD < & %%%
\DIFdelFL{853 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{on-stage interaction }%DIFDELCMD < & %%%
\DIFdelFL{528 }%DIFDELCMD < & %%%
\DIFdelFL{149 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{blackboard-writing }%DIFDELCMD < & %%%
\DIFdelFL{821 }%DIFDELCMD < & %%%
\DIFdelFL{277 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{teacher }%DIFDELCMD < & %%%
\DIFdelFL{8}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{490 }%DIFDELCMD < & %%%
\DIFdelFL{3}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{228 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{stand }%DIFDELCMD < & %%%
\DIFdelFL{13}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{932 }%DIFDELCMD < & %%%
\DIFdelFL{4}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{967 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{screen }%DIFDELCMD < & %%%
\DIFdelFL{5}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{025 }%DIFDELCMD < & %%%
\DIFdelFL{1}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{959 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{TeacherBehavior }%DIFDELCMD < & %%%
\DIFdelFL{blackboard }%DIFDELCMD < & %%%
\DIFdelFL{7}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{847 }%DIFDELCMD < & %%%
\DIFdelFL{3}%DIFDELCMD < {%%%
\DIFdelFL{,}%DIFDELCMD < }%%%
\DIFdelFL{445 }%DIFDELCMD < \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \textbf{BowTurnHead} contains images of students exhibiting two
...
the index only\DIFaddbegin \DIFadd{. The complete label-index mapping for each sub-dataset
is provided in Table~\ref{tab:index_mapping_audit}, which also serves
as an audit trail for index-order consistency checks.
The prompt template is}\DIFaddend :
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Label-index order audit for the index-only prompt. The same
...
%DIFDELCMD <     \item %%%
\DIFdelend \DIFaddbegin \noindent\DIFaddend \textbf{None (unfiltered):} All crops for which Qwen
produces a valid index are retained.
The pipeline uses the Qwen prediction as the pseudo-label.

\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin \noindent\DIFaddend \textbf{Valid-Output-Qwen:} Only crops where Qwen outputs
a valid in-range index are retained (identical to None in
practice, as Qwen has a near-100\% valid-output rate).

\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin \noindent\DIFaddend \textbf{Valid-Output-LLaVA:} Only crops where LLaVA outputs
a valid index are retained.

\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin \noindent\DIFaddend \textbf{Agreement:} Only crops where Qwen and LLaVA assign
...
%DIFDELCMD <     \item %%%
\DIFdelend \DIFaddbegin \noindent\DIFaddend \textbf{None:} Unfiltered MLLM pseudo-labels (Qwen predictions).
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin 

\noindent\DIFaddend \textbf{Agreement:} Agreement-filtered pseudo-labels.
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin 

\noindent\DIFaddend \textbf{GT:} Ground-truth human annotations (supervised reference).
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Seed policy by analysis module.
...
\DIFdelbegin \DIFdel{study includes a
}\DIFdelend \DIFaddbegin \DIFadd{revised study includes an expanded }\DIFaddend cross-model robustness check with \DIFdelbegin \DIFdel{three further }\DIFdelend \DIFaddbegin \DIFadd{ten
}\DIFaddend open-weight MLLMs \DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{spanning three model families (Qwen2, Qwen2.5, Qwen3.5,
...
Qwen2-VL-7B-Instruct, LLaVA-1.5-7B, }\DIFaddend Qwen2.5-VL-7B-Instruct,
Qwen2.5-VL-32B-Instruct, \DIFdelbegin \DIFdel{and Gemma-3-27B-IT.
The Gemma run loads the quantized Hugging Face checkpoint
}\texttt{\DIFdel{unsloth/gemma-3-27b-it-bnb-4bit}}%DIFAUXCMD
\DIFdel{, which implements the same
}\DIFdelend \DIFaddbegin \DIFadd{Qwen3.5-27B, Qwen3.5-35B-A3B (MoE),
Qwen3.6-27B, Qwen3.6-35B-A3B (MoE, FP8), }\DIFaddend Gemma-3-27B-IT \DIFdelbegin \DIFdel{model family used for this comparison.
These }\DIFdelend \DIFaddbegin \DIFadd{(4-bit),
Gemma4-26B-A4B-it (MoE), and Gemma4-31B-it.
All }\DIFaddend runs use the same bbox-crop protocol, index-only prompt, and
label decoding as the main annotation pipeline.
Validation annotations are collected for all three sub-datasets\DIFdelbegin \DIFdel{, and
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend TeacherBehavior is additionally processed on the training split to
...
on pairwise agreement-subset accuracy among model pairs\DIFdelbegin \DIFdel{.
Its purpose is not to redefine the main pipeline, but to test whether
the
main conclusions about model dependence and intent-dependent
failure remain stable under model substitution}\DIFdelend \DIFaddbegin \DIFadd{, and the
expanded model set allows testing whether the observed failure patterns
generalise across model generations, architectures, and scales}\DIFaddend .
...
cross-model consistency (CMC) score computed from the \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{ten }\DIFaddend MLLM
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{MLLM pseudo-label annotation accuracy and retention rate
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{BowTurnHead validation-set confusion counts for Qwen and
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Per-category MLLM annotation accuracy on TeacherBehavior
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Validation accuracy (\%) of CLIP ViT-L/14 under all
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Auxiliary filtering and teacher--student baselines under the
...
After the main downstream comparison, \DIFdelbegin \DIFdel{Table~\ref{tab:crossmodel_overall}
summarises }\DIFdelend \DIFaddbegin \DIFadd{Tables~\ref{tab:crossmodel_overall}
and~\ref{tab:crossmodel_intent} summarise }\DIFaddend validation-set annotation accuracy
for the \DIFdelbegin \DIFdel{five }\DIFdelend \DIFaddbegin \DIFadd{ten }\DIFaddend tested MLLMs under the common bbox-crop and index-only prompting
protocol.
...
the original two-model pair\DIFdelbegin \DIFdel{.
The comparisonshows that }\DIFdelend \DIFaddbegin \DIFadd{, across model generations, architectures, and
parameter scales.
}

\DIFadd{Three findings emerge from the expanded comparison.
First, }\DIFaddend no single annotator is uniformly best across datasets: \DIFdelbegin \DIFdel{Qwen2}\DIFdelend \DIFaddbegin \DIFadd{Qwen3}\DIFaddend .5\DIFdelbegin \DIFdel{-7B }\DIFdelend \DIFaddbegin \DIFadd{-35B-A3B
}\DIFaddend performs best on BowTurnHead \DIFaddbegin \DIFadd{(89.39\%)}\DIFaddend , Qwen2 \DIFdelbegin \DIFdel{performs best
on HandriseReadWrite , and }\DIFdelend \DIFaddbegin \DIFadd{on HandriseReadWrite (70.59\%),
and Qwen3.5-27B on TeacherBehavior (52.19\%).
Second, a clear capability boundary exists on the }\textit{\DIFadd{guide}} \DIFadd{category:
earlier-generation models (Qwen2, }\DIFaddend Qwen2\DIFaddbegin \DIFadd{.5, LLaVA, Gemma3-27B) all score below
25\%, while newer-generation models (Qwen3}\DIFaddend .5\DIFdelbegin \DIFdel{-7B and Gemma-3-27B are nearly tied on TeacherBehavior .
At the pairwise level, agreement-subset accuracy varies substantially
with the model pair, ranging from 22.41\% to 89.58\% on BowTurnHead, 57.60\% to 78.14\% on HandriseReadWrite, and 40.43\%to 49.47\%on
TeacherBehavior.
This pattern indicates that agreement quality is model-pair dependent
not a model-independent denoising effect}\DIFdelend \DIFaddbegin \DIFadd{-27B, Gemma4-31B) achieve 39--83\%,
showing that }\textit{\DIFadd{guide}} \DIFadd{is a model-capacity bottleneck rather than a
task-level impossibility.
Third, the }\textit{\DIFadd{answer}} \DIFadd{category remains below 19\% across all ten models
(the best is Qwen3.5-35B at 18.9\%), consistent with a task-level
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Five-model }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Ten-model }\DIFaddendFL cross-model robustness check of validation-set annotation
accuracy under a common bbox-crop and index-only prompting protocol.
\DIFaddbeginFL \DIFaddFL{Abbreviations: Q2.5-7B = }\DIFaddendFL Qwen2\DIFaddbeginFL \DIFaddFL{.5-VL-7B-Instruct;
G3-27B }\DIFaddendFL = \DIFdelbeginFL \DIFdelFL{Qwen2-VL-7B-Instruct}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Gemma-3-27B-IT (4-bit)}\DIFaddendFL ;
\DIFdelbeginFL \DIFdelFL{LLaVA }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Q3.5-27B }\DIFaddendFL = \DIFdelbeginFL \DIFdelFL{LLaVA-1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Qwen3}\DIFaddendFL .5\DIFdelbeginFL \DIFdelFL{-7B}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-27B}\DIFaddendFL ; \DIFdelbeginFL \DIFdelFL{Qwen2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Q3}\DIFaddendFL .5\DIFdelbeginFL \DIFdelFL{-7B }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-35B }\DIFaddendFL = \DIFdelbeginFL \DIFdelFL{Qwen2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Qwen3}\DIFaddendFL .5\DIFdelbeginFL \DIFdelFL{-VL-7B-Instruct}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-35B-A3B (MoE)}\DIFaddendFL ;
\DIFdelbeginFL \DIFdelFL{Qwen2.5-32B }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Q3.6-27B }\DIFaddendFL = \DIFaddbeginFL \DIFaddFL{Qwen3.6-27B; Q3.6-35B = Qwen3.6-35B-A3B (MoE, FP8);
G4-26B = Gemma4-26B-A4B-it (MoE); G4-31B = Gemma4-31B-it.
}\DIFaddendFL Qwen2 \DIFdelbeginFL \DIFdelFL{.5-VL-32B-Instruct}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{= Qwen2-VL-7B-Instruct}\DIFaddendFL ; \DIFdelbeginFL \DIFdelFL{Gemma-3-27B }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{LLaVA }\DIFaddendFL = \DIFdelbeginFL \DIFdelFL{Gemma-3-27B-IT}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{LLaVA-1.5-7B.
``---'' indicates the model was not evaluated on that dataset}\DIFaddendFL .
Values are bbox-level pseudo-label accuracy (\%). \DIFaddbeginFL \DIFaddFL{Best per dataset in bold.}\DIFaddendFL }
\label{tab:crossmodel_overall}
\centering
\scriptsize
\DIFdelbeginFL %DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD < \begin{tabular}{lccccccl}
%DIFDELCMD < \toprule
%DIFDELCMD < \textbf{Sub-dataset} & \textbf{Qwen2} & \textbf{LLaVA} & \textbf{Qwen2.5-7B} & \textbf{Qwen2.5-32B} & \textbf{Gemma-3-27B} & \textbf{Best} \\
%DIFDELCMD < \midrule
%DIFDELCMD < BowTurnHead       & 82.55 & 14.76 & \textbf{84.86} & 63.78 & 75.19 & Qwen2.5-7B \\
%DIFDELCMD < HandriseReadWrite & \textbf{70.59} & 53.20 & 51.74 & 69.11 & 57.21 & Qwen2 \\
%DIFDELCMD < TeacherBehavior   & 41.21 & 37.39 & \textbf{44.86} & 36.69 & 44.65 & Qwen2.5-7B / Gemma-3-27B \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\toprule
\textbf{Dataset} & \textbf{Qwen2} & \textbf{LLaVA} & \textbf{Q2.5-7B} & \textbf{G3-27B} & \textbf{Q3.5-27B} & \textbf{Q3.5-35B} & \textbf{Q3.6-27B} & \textbf{Q3.6-35B} & \textbf{G4-26B} & \textbf{G4-31B} \\
\midrule
BowTurnHead       & 82.55 & 14.76 & 84.71 & 75.19 & 79.48 & \textbf{89.37} & 74.66 & 63.26 & 89.61 & 86.36 \\
HandriseReadWrite & \textbf{70.59} & 53.20 & 51.16 & 57.21 & 59.25 & 62.66 & 61.03 & 69.11 & 55.84 & 56.90 \\
TeacherBehavior   & 41.21 & 37.39 & 44.84 & 44.65 & \textbf{52.21} & 40.33 & 48.67 & 36.69 & 45.10 & 43.51 \\
\bottomrule
\end{tabular}
}
\DIFaddendFL \normalsize
...
Table~\ref{tab:crossmodel_intent} \DIFdelbegin \DIFdel{isolates the two clearest
intent-dependent failure classes in TeacherBehavior .
The }\DIFdelend \DIFaddbegin \DIFadd{reports per-category accuracy for
four key TeacherBehavior categories that illustrate the generational and
task-level boundaries.
The }\DIFaddend \textit{\DIFdelbegin \DIFdel{answer}\DIFdelend \DIFaddbegin \DIFadd{guide}\DIFaddend } category \DIFdelbegin \DIFdel{remains near-failure across all five
models (0.00\% --5.16\%)}\DIFdelend \DIFaddbegin \DIFadd{shows a clear generational divide: earlier-generation
models (Qwen2}\DIFaddend , \DIFaddbegin \DIFadd{Qwen2.5, LLaVA, Gemma3-27B) all score below 1\% except LLaVA
(24.05\%, likely a model-specific bias), while newer-generation models reach
0.9--82.9\%, with Gemma4-31B achieving 82.9\%.
}\textit{\DIFadd{Answer}} \DIFadd{remains a near-universal failure: all ten models score
below 19\%, with the best (Qwen3.5-35B at 18.9\%) still far below
usable annotation quality---consistent with a task-level single-frame
observability constraint.
}\textit{\DIFadd{Blackboard-writing}} \DIFaddend and \DIFdelbegin \textit{\DIFdel{guide}} %DIFAUXCMD
\DIFdel{remains very weak overall }\DIFdelend \DIFaddbegin \textit{\DIFadd{screen}} \DIFadd{are reliably annotated
across all models }\DIFaddend (\DIFdelbegin \DIFdel{0.00\%--24.05\%), with four of the five models below 1\%.
The exception is LLaVA-1}\DIFdelend \DIFaddbegin \DIFadd{89.9--98.9\% and 48.1--88.8\%, respectively),
consistent with their strong visual signatures.
Qwen3}\DIFaddend .5\DIFdelbegin \DIFdel{-7B (24.05\%on }\textit{\DIFdel{guide}}%DIFAUXCMD
\DIFdel{), which is
more consistent with a model-specific decision bias than robust
intent-cue recognition, because the same model achieves only 14.76\%overall on BowTurnHead.
The same near-collapse persists under substantial model
substitution, strengthening the interpretation that these labels are
difficult to
resolve from single-frame cropsunder this task
setup.
}\DIFdelend \DIFaddbegin \DIFadd{-27B is the most balanced annotator on TeacherBehavior, with the
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Cross-model validation on the two clearest intent-dependent
TeacherBehavior categories.
Values are validation-set bbox-level accuracy (\%).
n = number of validation crops for the category.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\caption{\DIFaddFL{Ten-model cross-model validation on four key TeacherBehavior
categories illustrating the capability boundary (}\textit{\DIFaddFL{guide}}\DIFaddFL{) and the
task-level constraint (}\textit{\DIFaddFL{answer}}\DIFaddFL{).
Values are validation-set bbox-level accuracy (\%).
$n$ = number of validation crops.
Q3.6-27B values sourced from log files (result directory lost).}}
\DIFaddendFL \label{tab:crossmodel_intent}
\centering
\DIFdelbeginFL %DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD < \begin{tabular}{lcccccc}
%DIFDELCMD < \toprule
%DIFDELCMD < \textbf{Category} & \textbf{n} & \textbf{Qwen2} & \textbf{LLaVA} & \textbf{Qwen2.5-7B} & \textbf{Qwen2.5-32B} & \textbf{Gemma-3-27B} \\
%DIFDELCMD < \midrule
%DIFDELCMD < guide  & 449 & 0.22 & 24.05 & 0.00 & 0.89 & 0.22 \\
%DIFDELCMD < answer & 853 & 0.00 & 0.00 & 1.17 & 5.16 & 1.41 \\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\toprule
\textbf{Category} & \textbf{n} & \textbf{Qwen2} & \textbf{LLaVA} & \textbf{Q2.5-7B} & \textbf{G3-27B} & \textbf{Q3.5-27B} & \textbf{Q3.5-35B} & \textbf{Q3.6-27B} & \textbf{Q3.6-35B} & \textbf{G4-26B} & \textbf{G4-31B} \\
\midrule
guide               & 449 & 0.22 & 24.05 & 0.00 & 0.22 & 52.34 & 24.50 & 39.20 & 0.89 & 36.04 & \textbf{82.85} \\
answer              & 853 & 0.00 & 0.00 & 1.17 & 1.41 & 17.70 & 18.87 & 8.60 & 5.16 & 0.35 & 9.38 \\
blackboard-writing  & 277 & 98.56 & 36.82 & 95.67 & 70.04 & 98.19 & 98.56 & \textbf{98.90} & 85.92 & 94.58 & 95.67 \\
screen              & 1959 & 69.37 & 13.78 & 56.37 & 56.76 & \textbf{88.77} & 73.51 & 74.30 & 35.53 & 52.83 & 48.14 \\
\bottomrule
\end{tabular}
}
\normalsize
...
% 5. Discussion
%=================================================================
\DIFaddbegin \subsubsection{\DIFadd{Quality-Threshold Experiment with Qwen3.5-27B}}

\DIFadd{The cross-model validation identifies Qwen3.5-27B as the most balanced
...
\begin{table}[!ht]
\caption{\DIFaddFL{TeacherBehavior fine-tuning results comparing Qwen2-VL-7B
pseudo-labels (annotation accuracy 41.2\%) and Qwen3.5-27B pseudo-labels
(annotation accuracy 50.3\%).
The prompt-ensemble zero-shot baseline is 53.43\%.
GT upper bounds: LP 69.56\%, LoRA 74.71\%.}}
\label{tab:quality_threshold}
\centering
\begin{tabular}{lccc}
\toprule
\DIFaddFL{\textbf{Training condition}} & \DIFaddFL{\textbf{Qwen2 (41.2\%)}} & \DIFaddFL{\textbf{Qwen3.5-27B (50.3\%)}} & \DIFaddFL{$\boldsymbol{\Delta}$ }\\
\midrule
\DIFaddFL{None + LP  }& \DIFaddFL{42.27 }& \DIFaddFL{55.29 }& \DIFaddFL{+13.02 }\\
\DIFaddFL{None + LoRA }& \DIFaddFL{44.34 }& \DIFaddFL{53.79 }& \DIFaddFL{+9.45 }\\
\DIFaddFL{Selective + LP }& \DIFaddFL{58.37 }& \DIFaddFL{62.10 }& \DIFaddFL{+3.73 }\\
\DIFaddFL{Selective + LoRA }& \DIFaddFL{--- }& \DIFaddFL{53.20 }& \DIFaddFL{--- }\\
\midrule
\emph{\DIFaddFL{Prompt-ensemble ZS}} & \emph{\DIFaddFL{53.43}} & \emph{\DIFaddFL{53.43}} & \DIFaddFL{--- }\\
\emph{\DIFaddFL{GT + LP}} & \emph{\DIFaddFL{69.50}} & \emph{\DIFaddFL{---}} & \DIFaddFL{--- }\\
\bottomrule
\end{tabular}
\end{table}

\DIFadd{Figure~\ref{fig:quality_threshold} visualises the Qwen2 vs.\ Qwen3.5-27B
...
\caption{\DIFaddFL{TeacherBehavior validation accuracy under Qwen2-VL-7B pseudo-labels
(annotation accuracy 41.2\%) versus Qwen3.5-27B pseudo-labels (50.3\%).
The dashed line marks the prompt-ensemble zero-shot baseline (53.43\%).
...
\DIFadd{Three findings from this experiment are directly relevant to the paper's
central question.
...
zero-shot baseline (None+LP: 42.27\% vs.\ 53.43\% prompt-ensemble zero-shot), while
Qwen3.5-27B labels (50.3\%) produce accuracy above it (None+LP: 55.29\%
vs.\ 53.43\% prompt-ensemble zero-shot).
...
\DIFadd{Second, the selective-routing gain scales with annotator quality.
Under Qwen2-VL-7B pseudo-labels, selective+LP improves over None+LP
by 16.1pp (58.37\% vs.\ 42.27\%).
...
\DIFadd{Third, LoRA remains unstable in the TeacherBehavior selective setting.
...
The \DIFdelbegin \DIFdel{five-model }\DIFdelend \DIFaddbegin \DIFadd{ten-model }\DIFaddend boundary check strengthens this interpretation.
...
\subsection{Finding 3: \DIFdelbegin \DIFdel{Anchor-Aware Selective }\DIFdelend Annotation \DIFdelbegin \DIFdel{Provides an
Informative Upper-Bound Diagnostic}\DIFdelend \DIFaddbegin \DIFadd{Quality Thresholds and the Selective-Routing Upper Bound}\DIFaddend }

\DIFdelbegin \DIFdel{The selective-annotation experiment addresses a narrower question than
the main matrix: if some categories are visually anchorable and others
are not, is uniform }\DIFdelend \DIFaddbegin \DIFadd{Two related findings constrain when }\DIFaddend pseudo-label \DIFdelbegin \DIFdel{treatment the wrong abstraction?
Table~\ref{tab:selective_results} suggests that uniform treatment may
be inadequate.
Under oracle routing, assigning high-anchor categories to }\DIFdelend \DIFaddbegin \DIFadd{supervision is likely to
be useful.
First, the quality-threshold experiment (Table~\ref{tab:quality_threshold})
identifies the annotation accuracy range at which }\DIFaddend pseudo-label \DIFdelbegin \DIFdel{supervision while leaving low-anchor categories to the }\DIFdelend \DIFaddbegin \DIFadd{fine-tuning
crosses the }\DIFaddend zero-shot \DIFdelbegin \DIFdel{branch
scores above both full pseudo-label fine-tuning and }\DIFdelend \DIFaddbegin \DIFadd{baseline.
Qwen2-VL-7B labels at 41.2\% annotation accuracy produce downstream
accuracy below zero-shot (42.27\% vs.\ 53.43\% prompt-ensemble), while Qwen3.5-27B
...
\DIFadd{Second, }\DIFaddend the \DIFaddbegin \DIFadd{selective-routing diagnostic (Table~\ref{tab:selective_results})
shows that its benefit scales with base annotation quality.
Selective+LP improves over }\DIFaddend zero-shot \DIFdelbegin \DIFdel{baseline}\DIFdelend \DIFaddbegin \DIFadd{by 4.94pp with Qwen2 labels and by
8.67pp with Qwen3.5-27B labels (62.10\% vs.\ 53.43\%).
This supports the interpretation that anchor-aware supervision routing
amplifies, rather than replaces, the benefit of higher-quality annotations}\DIFaddend .
The result \DIFdelbegin \DIFdel{should still be read as }\DIFdelend \DIFaddbegin \DIFadd{remains }\DIFaddend a boundary diagnostic, not a deployable method\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, because
}\DIFaddend Scheme~B uses oracle routing \DIFdelbegin \DIFdel{at evaluation time, and the current
AnchorProxy is operationally }\DIFdelend \DIFaddbegin \DIFadd{and the AnchorProxy is }\DIFaddend coupled to the
zero-shot branch\DIFdelbegin \DIFdel{used for
low-anchor classes.
Its value is diagnostic: it identifies an upper-bound regime
in which pseudo-label supervision may be most defensible for categories
with sufficient visual evidence}\DIFdelend .
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[!ht]
\DIFaddendFL \caption{Comparison of annotation strategies on TeacherBehavior
...
deployable MLLMs, and the \DIFdelbegin \DIFdel{broader }\DIFdelend \DIFaddbegin \DIFadd{expanded cross-model }\DIFaddend robustness analysis spans
\DIFdelbegin \DIFdel{five
models in total}\DIFdelend \DIFaddbegin \DIFadd{ten models across three families (Qwen, LLaVA, Gemma)}\DIFaddend : Qwen2-VL-7B, LLaVA-1.5-7B,
Qwen2.5-VL-7B-Instruct, Qwen2.5-VL-32B-Instruct, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{Qwen3.5-27B,
Qwen3.5-35B-A3B, Qwen3.6-27B, Qwen3.6-35B-A3B, }\DIFaddend Gemma-3-27B-IT\DIFaddbegin \DIFadd{,
Gemma4-26B-A4B-it, and Gemma4-31B-it}\DIFaddend .
...
The \DIFdelbegin \DIFdel{five-model robustness analysis partly strengthens }\DIFdelend \DIFaddbegin \DIFadd{ten-model robustness analysis and the Qwen3.5-27B quality-threshold
experiment partly strengthen }\DIFaddend external validity with respect to annotator
choice, but \DIFdelbegin \DIFdel{it does not remove this internal validity limitation }\DIFdelend \DIFaddbegin \DIFadd{they do not remove the internal validity limitations }\DIFaddend in the
...
The \DIFdelbegin \DIFdel{repository implements these analysis modules using }\DIFdelend \DIFaddbegin \DIFadd{analysis modules described in this paper are implemented as }\DIFaddend the
following scripts at the repository root:
\DIFaddbegin \path{step0b_cape_zeroshot.py} \DIFadd{(}\DIFaddend zero-shot evaluation\DIFdelbegin \DIFdel{(baseline and prompt-ensemble settings,
}%DIFDELCMD < \path{step0b_cape_zeroshot.py}%%%
\DIFdel{),
MLLM annotation (}\DIFdelend \DIFaddbegin \DIFadd{),
}\DIFaddend \path{step1_llm_annotate.py} and \path{cross_model_annotate.py}
\DIFaddbegin \DIFadd{(MLLM annotation),
}\path{step2_filter_analysis.py} \DIFadd{(filtering analysis}\DIFaddend ),
\DIFdelbegin \DIFdel{filtering analysis }\DIFdelend \DIFaddbegin \path{step3_clip_finetune.py} \DIFaddend (\DIFdelbegin %DIFDELCMD < \path{step2_filter_analysis.py}%%%
\DIFdel{),
}\DIFdelend CLIP fine-tuning\DIFaddbegin \DIFadd{),
}\path{step4_selective_annotation.py} \DIFadd{(selective annotation),
}\path{step5_retention_curve.py} \DIFaddend (\DIFdelbegin %DIFDELCMD < \path{step3_clip_finetune.py}%%%
\DIFdel{),
selective annotation (}%DIFDELCMD < \path{step4_selective_annotation.py}%%%
\DIFdel{),
}\DIFdelend retention-ratio curve\DIFdelbegin \DIFdel{(}%DIFDELCMD < \path{step5_retention_curve.py}%%%
\DIFdelend ),
and \DIFaddbegin \path{summarize_repeated_seeds.py} \DIFadd{(}\DIFaddend repeated-seed aggregation\DIFdelbegin \DIFdel{(}%DIFDELCMD < \path{summarize_repeated_seeds.py}%%%
\DIFdel{).
The auxiliary strategy and consistency }\DIFdelend \DIFaddbegin \DIFadd{).
Auxiliary }\DIFaddend analyses are implemented in
\path{confidence_filtering.py},
\path{teacher_student_self_training.py},
and \path{cross_model_consistency.py}.
The \DIFdelbegin \DIFdel{submitted artifact includes }%DIFDELCMD < \path{environment.yml} %%%
\DIFdel{and
}\DIFdelend \DIFaddbegin \DIFadd{runtime environment is specified in }\path{environment.yml}
\DIFadd{and }\DIFaddend \path{requirements.txt}\DIFdelbegin \DIFdel{for environment reconstruction. The experiment
snapshot used for manuscript preparation is identified by repository
commit }\texttt{\DIFdel{e21ae3d09c8d1577eeb75d1a29e2661cf757e877}}%DIFAUXCMD
\DIFdel{; the archived
...
(seed 42)}\DIFdelend .
Table-level provenance\DIFdelbegin \DIFdel{is as follows}\DIFdelend : annotation-quality and confusion tables are
\DIFdelbegin \DIFdel{generated }\DIFdelend \DIFaddbegin \DIFadd{produced }\DIFaddend by the filtering-analysis module\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend the main fine-tuning matrix
by the \DIFdelbegin \DIFdel{CLIP }\DIFdelend fine-tuning and repeated-seed summary modules\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend the
selective-routing \DIFdelbegin \DIFdel{table }\DIFdelend \DIFaddbegin \DIFadd{results }\DIFaddend by the selective-annotation module\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend the
auxiliary-baseline \DIFdelbegin \DIFdel{table }\DIFdelend \DIFaddbegin \DIFadd{results }\DIFaddend by the confidence-filtering and
teacher--student modules\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend and the retention-ratio figure by the
retention-curve module.
The SCB dataset is publicly available~\cite{scb_dataset}.
The \DIFdelbegin \DIFdel{code repository and run configurationsused for the analysis are publicly available }\DIFdelend \DIFaddbegin \DIFadd{complete codebase, run configurations, and generated result files
are archived }\DIFaddend at
\url{https://github.com/zhanglizhuo/llm-annotation}
(accessed on \DIFdelbegin \DIFdel{27 April }\DIFdelend \DIFaddbegin \DIFadd{25 June }\DIFaddend 2026), \DIFdelbegin \DIFdel{including the pipeline scripts, archived
...
accessible and preserve per-module run recipes for the main tables and figures}\DIFdelend \DIFaddbegin \DIFadd{pinned at commit }\texttt{\DIFadd{e21ae3d}}\DIFadd{.
LP main-matrix results are averaged over repeated seeds
(all datasets: five seeds, 43--47); targeted LoRA replications
for BowTurnHead and TeacherBehavior under None and GT use
seeds 43--47, while remaining LoRA and selected diagnostics
are single-seed or fixed-seed evaluations (seed 42)}\DIFaddend .
...
The \DIFaddbegin \DIFadd{quality-threshold experiment locates the critical annotation accuracy
range at which pseudo-label fine-tuning crosses the zero-shot baseline on
TeacherBehavior (between 41\% and 50\%), and the expanded ten-model
cross-model validation reveals a generational boundary on
}\textit{\DIFadd{guide}} \DIFadd{while confirming that }\textit{\DIFadd{answer}} \DIFadd{remains below 19\%
across all tested models.
The }\DIFaddend CMC diagnostic, the auxiliary confidence-filtering and
teacher--student baselines, the oracle Scheme~B diagnostic, and the
\DIFdelbegin \DIFdel{five-model }\DIFdelend \DIFaddbegin \DIFadd{ten-model }\DIFaddend robustness check then bound the claim: pseudo-label
...
\DIFaddbegin \DIFadd{select an annotator with task-level accuracy above the zero-shot
threshold (approximately 50\% for TeacherBehavior-level complexity),
}\DIFaddend use pseudo-label supervision mainly on high-anchor categories,
...
\DIFdelbegin \DIFdel{27 April }\DIFdelend \DIFaddbegin \DIFadd{25 June }\DIFaddend 2026). These datasets are externally released resources and are
not owned by this study. The experiment code and archived run
configurations are available at
\url{https://github.com/zhanglizhuo/llm-annotation} (accessed on \DIFdelbegin \DIFdel{27 April }\DIFdelend \DIFaddbegin \DIFadd{25 June }\DIFaddend 2026).
...
\DIFdelbegin \bibitem{deitke2024molmo}
\DIFdel{Deitke M, Clark C, Lee S, Tripathi S, Yang Y, Park J, et al.
Molmo and PixMo: Open weights and open data for state-of-the-art
multimodal models. arXiv }%DIFDELCMD < [%%%
\DIFdel{Preprint}%DIFDELCMD < ]%%%
\DIFdel{. 2024 }%DIFDELCMD < [%%%
\DIFdel{cited 2026 May 7}%DIFDELCMD < ]%%%
\DIFdel{. Available
from: https://arxiv.org/abs/2409.17146
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem{chen2024internvl2}
...
%DIF > =======================
%DIF >  Author Biographies
%DIF > =======================
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{mayan.jpg}}]{YAN MA}
\DIFadd{is an Associate Professor at the School of Foreign Studies, Changsha
University of Science and Technology. Her research interests include
second language acquisition and teaching English as a second language.
She has published many articles in Chinese peer-reviewed journals. She
is pursuing a PhD in Science at Hunan Agricultural University, with a
research focus on utilizing information technology to analyze the
impact of classroom environment on students' engagement in English
learning through data analysis.
}\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{lizhuo.jpg}}]{Lizhuo Zhang}
\DIFadd{received the B.S. degree in Horticulture from Hunan Agricultural
University, Changsha, China, and the M.S. and Ph.D. degrees in
Computer Application Technology from Central South University,
Changsha, China. He was a Senior Research Associate with City
University of Hong Kong, Hong Kong SAR, China, a Postdoctoral
Research Fellow with University of Macau, Macau SAR, China, and a
Senior Software Engineer with Huawei Software Technologies Co., Ltd.,
China. He is currently a Lecturer with Hunan Agricultural University,
Changsha, China. His research interests include the Internet of
Things, artificial intelligence, and intelligent computing systems.
}\end{IEEEbiography}
\DIFaddend 
